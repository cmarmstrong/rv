...
#'   `class' (fips) to set the class of the returned geometry.  Both arguments
#'   accept a value of either `sf' or `sp'.
#' @return A vector for the administrative boundary identified by the location
#'   code
#' @export
v <- function(country=NULL, fips=NULL, hasc=NULL, cb=TRUE, resolution='20m', ...) {
    ...
    if(!is.null(hasc)) {
        ...
        states <- rnaturalearth::ne_states(country=country, ...)
        v <- states[states $code_hasc %in% hasc, ]
    } else if(!is.null(country)) {
        v <- rnaturalearth::ne_countries(country=country, ...)
    }
    if(!is.null(fips)) {
        state <- substr(fips, 1, 2)
        county <- substr(fips, 3, 5)
        counties <- tigris::counties(state=unique(state), cb, resolution, ...)
